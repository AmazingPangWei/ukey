...
`请求/report/listReport,数据类型:ajax/json，请求参数：key:page value:请求的第几页，返回key:total value:一共举报的数量  key:reports value:List<ReportResult> 其中ReportResult对象参数分别为uname---被举报用户的名字，data---被举报日期，reason---被举报理由`
